...
| 桌面 EXE 壳（`src/`、`scripts/`、`tests/`，本仓库原创） | ≈ 2,500 行 | 独立新增 |

> 说明：桌面 EXE 壳为独立新增代码，不计入上游占比；表中「占比」仅就 `harness/` 目录内的上游代码与修改计算。
...
### 3.2 桌面 EXE 壳（仓库根目录，原创）

1. Electron 桌面壳：加载本地 DSH Web GUI（仅允许 `127.0.0.1` 白名单地址），屏蔽浏览器快捷键与手势干扰。
