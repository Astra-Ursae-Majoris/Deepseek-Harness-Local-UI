...
export interface IApiClient {
  sessions: {
    ...
    outline(payload: RequestPayload<'session.outline'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'session.outline'>>>
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'session.outline': sessionOutlineValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    outline: (payload, signal) => this.callUnary('session.outline', payload, signal),
