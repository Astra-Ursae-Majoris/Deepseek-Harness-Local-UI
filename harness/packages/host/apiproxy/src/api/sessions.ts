...
/** One conversation-outline row: the turn's first user message anchor plus one-line summaries. */
export interface OutlineTurn {
  /** Timeline turn id. */
  turn: number
  /** Seq of the turn's first user message (the jump anchor; falls back to the turn/start seq). */
  anchorSeq: number
  /** Turn/start wall-clock epoch ms. */
  time: number
  /** First user text (joined text blocks, code-point bounded). */
  userText: string
  /** First assistant text (code-point bounded). */
  replyText: string
  /** Whether the turn has a logged turn/end. */
  status: 'open' | 'closed'
  /** Whether the turn contains a tool/call event. */
...
export interface SessionsApi {
  ...
  /**
   * Returns the full conversation outline: one row per turn with first-line
   * user/assistant summaries, independent of the client's loaded window, so
   * a sidebar can list and jump to any turn of the session.
   */
  outline(
    request: RpcRequest<{ sessionId: SessionId }>,
    signal: AbortSignal,
  ): Promise<RpcResponse<{ turns: OutlineTurn[] }>>
