...
import type {
  ...
  ModelReasoningEffort, ModelSelection, OutlineTurn, SessionListMetadata, SessionProjectionsBlock,
...
/** Fixed wire bound for one outline text field (the client truncates for display). */
const OUTLINE_TEXT_MAX_CODE_POINTS = 240

/** session.outline request payload. */
export const sessionOutlineRequestSchema = z.object({
  sessionId: sessionIdSchema,
}) satisfies z.ZodType<Wire<RequestPayload<'session.outline'>>>

/** One session.outline row. */
export const sessionOutlineTurnSchema = z.object({
  turn: z.number().int().nonnegative(),
  anchorSeq: z.number().int().nonnegative(),
  time: z.number(),
  userText: z.string().refine(
    text => truncateUnicodeCodePoints(text, OUTLINE_TEXT_MAX_CODE_POINTS) === text,
    { message: `outline user text must contain at most ${OUTLINE_TEXT_MAX_CODE_POINTS} Unicode code points` },
  ),
  replyText: z.string().refine(
    text => truncateUnicodeCodePoints(text, OUTLINE_TEXT_MAX_CODE_POINTS) === text,
    { message: `outline reply text must contain at most ${OUTLINE_TEXT_MAX_CODE_POINTS} Unicode code points` },
  ),
  status: z.enum(['open', 'closed']),
  hasToolCalls: z.boolean(),
}) satisfies z.ZodType<Wire<OutlineTurn>>

/** session.outline response value. */
export const sessionOutlineValueSchema = z.object({
  turns: z.array(sessionOutlineTurnSchema),
}) satisfies z.ZodType<Wire<ResponseValue<'session.outline'>>>
