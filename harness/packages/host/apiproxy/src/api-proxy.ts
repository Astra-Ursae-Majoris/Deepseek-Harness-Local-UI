...
import type {
  ...
  ModelReasoning, MuxFrame, OutlineTurn, PromptContentPart, QuestionResponsePayload, SessionListMetadata, SessionProjectionsBlock, SessionSearchItem,
...
/** Join the text blocks of a user/assistant message payload (messages always carry content). */
function messageText(content: readonly { type?: unknown; text?: unknown }[]): string {
  let out = ''
  ...
  return {
    sessions: {
      ...
      async outline(request, signal) {
        const cancelled = () => err<{ turns: OutlineTurn[] }>(request, {
          code: 'cancelled',
          message: 'session outline was aborted',
          details: {},
        })
        ...
        try {
          ...
          /* v8 ignore next -- persistence failures surface as session-not-found via the same read; the guard keeps alternate backends honest. */
          return err(request, {
            code: 'internal',
            message: `session outline source unavailable for session "${sessionId}": ${String(error)}`,
            details: {},
          })
        }
        if (isAborted(signal)) return cancelled()
        // One row per turn over the whole log: the client's loaded window
        // bounds neither the listing nor the jump targets.
        const turns: OutlineTurn[] = []
        let current: {
          turn: number
          startSeq: number
          firstUserSeq: number | undefined
          time: number
          userText: string
          replyText: string
        ...
        const settle = (): void => {
          ...
          turns.push({
            turn: current.turn,
            anchorSeq: current.firstUserSeq ?? current.startSeq,
            time: current.time,
            userText: truncateUnicodeCodePoints(current.userText, OUTLINE_TEXT_MAX_CODE_POINTS),
            replyText: truncateUnicodeCodePoints(current.replyText, OUTLINE_TEXT_MAX_CODE_POINTS),
            status: current.closed ? 'closed' : 'open',
            hasToolCalls: current.hasToolCalls,
          })
          current = undefined
        }
        for (const event of source.events) {
          switch (event.type) {
            ...
            case 'user/message': {
              ...
              current.firstUserSeq ??= event.seq
              const text = messageText(event.data.content)
              if (current.userText === '') current.userText = text
              break
            }
            case 'assistant/message': {
              if (current === undefined) break
              const text = messageText(event.data.message.content)
              if (current.replyText === '') current.replyText = text
              break
            }
            case 'tool/call': {
              if (current === undefined) break
            ...
            case 'turn/end': {
              if (current !== undefined && current.turn === event.data.turn) current.closed = true
              break
            }
            default:
              break
