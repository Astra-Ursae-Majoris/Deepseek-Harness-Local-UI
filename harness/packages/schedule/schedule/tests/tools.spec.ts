...
describe('Schedule tool protocol', () => {
  ...
  it('rejects shape-known invalid create input before persistence', async () => {
    ...
    expect(value(await execute(test, 'schedule_create', { prompt: 'x', weekly: {
      weekday: 0, time: '02:00:00', time_zone: 'Asia/Shanghai',
    } })))
      .toEqual({ code: 'invalid_rule', message: 'weekly must carry an integer weekday from 1 (Monday) to 7 (Sunday), a time string, and a time_zone string.' })
