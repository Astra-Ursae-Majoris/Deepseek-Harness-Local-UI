...
/** Resolve the latest due occurrence for one recurring record kind. */
function resolveRecurringOccurrence(record: RecurringScheduleRecord, now: number): { occurrenceAt: string } {
  return record.kind === 'every'
    ? resolveEveryOccurrence(record, now)
    : resolveWeeklyOccurrence(record, now)
}
...
function dueDecision(folded: FoldedSchedules, now: number): DueDecision {
  ...
  const oneShot = indexed
    .filter((entry): entry is { record: OneShotScheduleRecord; index: number } =>
      (entry.record.kind !== 'every' && entry.record.kind !== 'weekly')
  ...
  if (recurring.length > 0) {
    return {
      ...
      reminders: recurring.map(({ record }) => ({
        record,
        occurrenceAt: resolveRecurringOccurrence(record, now).occurrenceAt,
      })),
    }
  }
