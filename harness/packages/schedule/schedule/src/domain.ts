...
/** One week in milliseconds, used only to bound weekly recurrence searches. */
...
/** Parse one validated local wall-clock time into exact clock fields. */
function parseTimeOfDay(value: string): {
  ...
  if (groups === undefined) {
    throw new ScheduleInputError(
      'invalid_rule',
      'weekly time must use HH:mm:ss with optional one-to-three digit milliseconds.',
    )
  }
  const hour = groupNumber(groups, 'hour')
  const minute = groupNumber(groups, 'minute')
  const second = groupNumber(groups, 'second')
  if (hour > 23 || minute > 59 || second > 59) {
    throw new ScheduleInputError('invalid_rule', 'The weekly time must be a real local wall-clock time.')
  }
  return { hour, minute, second, millisecond: milliseconds(groups['fraction']) }
}
...
/** Construct the zone formatter shared by weekly local projections. */
...
/** Read the zone-local calendar date of one epoch. */
function localDateAt(formatter: Intl.DateTimeFormat, epoch: number): { year: number; month: number; day: number } {
  const projected = localProjection(formatter, epoch)
  return { year: projected.year, month: projected.month, day: projected.day }
}

/** Add a whole-day offset to one calendar date, normalizing month and year boundaries. */
function dateAfterDays(
  ...
  epoch.setUTCFullYear(base.year, base.month - 1, base.day + days)
  return { year: epoch.getUTCFullYear(), month: epoch.getUTCMonth() + 1, day: epoch.getUTCDate() }
}

/** ISO 8601 weekday of one calendar date: 1 (Monday) through 7 (Sunday). */
function isoWeekdayOf(date: { year: number; month: number; day: number }): number {
  const sundayZero = new Date(Date.UTC(date.year, date.month - 1, date.day)).getUTCDay()
  return ((sundayZero + 6) % 7) + 1
}

/** Resolve the first weekly occurrence strictly after `after`, skipping daylight-saving gaps. */
function nextWeeklyOccurrence(
  weekday: number,
  timeOfDay: { hour: number; minute: number; second: number; millisecond: number },
  timeZone: string,
  after: number,
): number | undefined {
  const formatter = zoneFormatter(timeZone)
  const base = localDateAt(formatter, after)
  for (let offset = 0; offset <= 8; offset += 1) {
    const date = dateAfterDays(base, offset)
    if (isoWeekdayOf(date) !== weekday) continue
    let target: number
    try {
      target = resolveLocalInstant({ ...date, ...timeOfDay }, timeZone)
    } catch (error: unknown) {
      if (error instanceof ScheduleInputError && error.code === 'invalid_rule') continue
      if (error instanceof ScheduleInputError && error.code === 'time_out_of_range') return undefined
      throw error
    }
...
function decodeWeeklyRecord(value: unknown): WeeklyScheduleRecord {
  if (!isRecord(value)
    || !hasExactKeys(value, ['id', 'kind', 'prompt', 'weekday', 'time', 'time_zone', 'scheduledAt'])) {
    throw new ScheduleLogError('weekly schedule must contain exactly id, kind, prompt, weekday, time, time_zone, and scheduledAt')
  }
  const prompt = value['prompt']
  if (typeof prompt !== 'string' || prompt.length === 0 || prompt.trim() !== prompt) {
  ...
  if (!Number.isSafeInteger(weekday) || (weekday as number) < 1 || (weekday as number) > 7) {
    throw new ScheduleLogError('weekly weekday must be a safe integer from 1 to 7')
  }
  const time = value['time']
  if (typeof time !== 'string' || !LOCAL_TIME.test(time)) {
    throw new ScheduleLogError('weekly time must be a valid local wall-clock time')
  }
  const timeZone = value['time_zone']
  if (typeof timeZone !== 'string') {
    throw new ScheduleLogError('weekly time_zone must be a string')
  }
  try {
    canonicalizeTimeZone(timeZone)
  } catch {
    throw new ScheduleLogError('weekly time_zone must be UTC or a valid IANA Area/Location name')
  }
  return Object.freeze({
    id: decodeId(value['id']),
    kind: 'weekly',
...
/**
 * Resolve one latest-only weekly decision without enumerating a backlog.
 * @param record - Active record whose target is the earliest unaccepted occurrence.
 * @param acceptedAt - Wall-clock decision time in epoch milliseconds.
 * @returns The latest due occurrence and first strictly future target, if representable.
 */
export function resolveWeeklyOccurrence(
  ...
  if (acceptedAt < anchor) {
    throw new ScheduleLogError('weekly dispatch cannot precede the active scheduledAt')
  }
  const timeOfDay = parseTimeOfDay(record.time)
  const estimate = anchor + Math.floor((acceptedAt - anchor) / WEEK_MS) * WEEK_MS
  let candidate = nextWeeklyOccurrence(record.weekday, timeOfDay, record.time_zone, estimate - 2 * WEEK_MS)
  let latest: number | undefined
  for (let guard = 0; guard < 8; guard += 1) {
    if (candidate === undefined || candidate > acceptedAt) break
    latest = candidate
    candidate = nextWeeklyOccurrence(record.weekday, timeOfDay, record.time_zone, candidate)
  }
  const occurrence = latest ?? anchor
  const occurrenceAt = new Date(occurrence).toISOString()
  const next = nextWeeklyOccurrence(record.weekday, timeOfDay, record.time_zone, occurrence)
  return Object.freeze({
    occurrenceAt,
...
/**
 * Validate a weekly rule and compute its first strictly-future occurrence.
 * @param id - Already allocated session-local id.
 * @param prompt - Reminder content supplied at creation.
 * @param weekday - ISO 8601 weekday, 1 (Monday) through 7 (Sunday).
 * @param time - Local wall-clock time with optional one-to-three digit milliseconds.
 * @param timeZone - UTC or IANA Area/Location zone that anchors the recurrence.
 * @param now - Single creation-time wall-clock sample in epoch milliseconds.
...
export function createWeeklyScheduleRecord(
  ...
  if (!Number.isSafeInteger(weekday) || weekday < 1 || weekday > 7) {
    throw new ScheduleInputError('invalid_rule', 'weekly weekday must be a safe integer from 1 (Monday) to 7 (Sunday).')
  }
  const timeOfDay = parseTimeOfDay(time)
  const canonical = canonicalizeTimeZone(timeZone)
  const target = nextWeeklyOccurrence(weekday, timeOfDay, canonical, now)
  if (target === undefined) {
    throw new ScheduleInputError(
      'time_out_of_range',
      'The weekly target must be representable as a four-digit-year RFC 3339 UTC instant.',
    )
  }
  return Object.freeze({
    id,
