...
const CREATE_DESCRIPTION =
  ...
  + 'or weekly as an object with an ISO weekday (1 Monday through 7 Sunday), a local wall-clock '
  + 'time, and an explicit IANA time zone. '
  + 'Recurring reminders skip missed occurrences and batch one latest '
...
function validateCreateArgs(args: {
  ...
  weekly?: { weekday: number; time: string; time_zone: string }
  ...
  if (args.weekly !== undefined && (
    !Number.isSafeInteger(args.weekly.weekday)
    || args.weekly.weekday < 1
    || args.weekly.weekday > 7
    || typeof args.weekly.time !== 'string'
    || typeof args.weekly.time_zone !== 'string'
  )) {
    return {
      code: 'invalid_rule',
      message: 'weekly must carry an integer weekday from 1 (Monday) to 7 (Sunday), a time string, and a time_zone string.',
    }
  }
    ...
    disposers.push(toolCtx.tools.register(defineTool({
      ...
      parameters: {
        ...
        weekly: {
          type: 'object',
          additionalProperties: false,
          description: 'Weekly recurrence as an ISO weekday (1 Monday through 7 Sunday), a local wall-clock time, and an explicit IANA time zone.',
          properties: {
        ...
        return runCancellableScheduleTransaction(agent, exec.signal, async () => {
          ...
          try {
            if (args.at !== undefined) {
              ...
              record = createWeeklyScheduleRecord(
                id,
                args.prompt,
                args.weekly.weekday,
                args.weekly.time,
                args.weekly.time_zone,
                Date.now(),
              )
            } else {
