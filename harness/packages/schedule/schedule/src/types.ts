...
/** Durable weekly reminder whose next target stays fixed to one ISO weekday and local wall-clock time. */
export interface WeeklyScheduleRecord {
  /** Session-local stable identity. */
  readonly id: ScheduleId
  /** Rule discriminator for a fixed-weekday recurring reminder. */
  ...
  /** Explicit UTC or IANA Area/Location zone that anchors the recurrence. */
  readonly time_zone: string
  /** Four-digit-year RFC 3339 UTC target of the earliest not-yet-dispatched occurrence. */
  readonly scheduledAt: string
}
...
/** Structured weekly recurrence input accepted by `schedule_create`. */
...
/** Recurring record variants that stay active across dispatches. */
