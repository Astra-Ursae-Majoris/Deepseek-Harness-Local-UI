...
import type {
  ...
  SessionListState, OutlineTurn, SessionProvideDescriptor, SessionSearchResultItem, SessionSummary, SnapshotStore,
  SubagentAddress,
...
export class TestSessions implements ISessions {
  ...
  /** Replaceable outline behavior (see {@link TestSessions.stubOutline}). */
  private outlineStub: ((sessionId: SessionId, signal: AbortSignal) => OutlineTurn[]) | undefined
  ...
  /**
   * Replace the conversation-outline result page (the call is still recorded).
   * @param impl - turn rows for a session, as the Host would derive them.
   */
  stubOutline(impl: (sessionId: SessionId, signal: AbortSignal) => OutlineTurn[]): void {
    this.outlineStub = impl
  }
  ...
  /**
   * Recorded outline stub: the default answers an empty outline, mirroring
   * the search face (Host derivation is out of scope for the test runtime).
   * @param sessionId - target session.
   * @param signal - cancellation for a superseded fetch (recorded and forwarded).
   * @returns the stubbed or empty turn list.
   */
  outline(sessionId: SessionId, signal: AbortSignal): ReturnType<ISessions['outline']> {
    this.calls.push({ method: 'outline', args: [sessionId, signal] })
    return Promise.resolve({ ok: true, value: { turns: this.outlineStub?.(sessionId, signal) ?? [] } })
  }
