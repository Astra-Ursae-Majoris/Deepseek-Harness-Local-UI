...
import type {
  OutlineTurn, RpcResult, SessionId, SubagentAddress,
...
export interface ISessions {
  ...
  /**
   * Fetch the full conversation outline (one row per turn with first-line
   * summaries), independent of the loaded window, so the sidebar can list
   * and jump to any turn of the session.
   * @param sessionId - target session.
   * @param signal - cancellation for a superseded fetch.
   * @returns the whole-log turn rows or a business/transport error.
   */
  outline(
    sessionId: SessionId,
    signal: AbortSignal,
