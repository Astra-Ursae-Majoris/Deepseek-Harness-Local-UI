...
import type {
  IApiClient, OutlineTurn, RpcError, RpcResult, SessionId, SubagentAddress, JobView, WorkspaceId,
...
export class SessionRuntime implements ISessions {
   ...
   * @returns the Host's whole-log turn rows or a business/transport error.
   */
  outline(
    sessionId: SessionId,
    signal: AbortSignal,
  ): Promise<RpcResult<{ turns: OutlineTurn[] }>> {
    return this.manager.outline(sessionId, signal)
  }
