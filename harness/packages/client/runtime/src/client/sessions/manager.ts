...
import type {
  IApiClient, HostFrame, MuxFrame, OutlineTurn, RpcError, RpcRequest, RpcResult, SessionId,
...
export class SessionManager {
  ...
  /**
   * Fetch the full conversation outline independent of the loaded window.
   * @param sessionId - target session.
   * @param signal - cancellation for a superseded fetch.
   * @returns the Host's whole-log turn rows or a folded transport error.
   */
  async outline(
    sessionId: SessionId,
    signal: AbortSignal,
  ): Promise<RpcResult<{ turns: OutlineTurn[] }>> {
    try {
      return (await this.api.sessions.outline({ sessionId }, signal)).result
    } catch (error: unknown) {
      return transportError(error)
    }
  }
