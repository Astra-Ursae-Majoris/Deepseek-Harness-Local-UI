...
import type {
  ...
  OutlineTurn, RpcError, RpcReceipt, RpcRequest, RpcResponse, SessionId, SessionModels, SessionSearchItem, SkillEntry,
...
export class FakeApiClient implements IApiClient {
  ...
  onOutline: (payload: unknown) => Promise<RpcResponse<{ turns: OutlineTurn[] }>> =
    () => Promise.resolve(ok({ turns: [] }))
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    outline: (payload: unknown, signal?: AbortSignal) => {
      this.lastSearchSignal = signal
      return this.record('session.outline', payload, this.onOutline(payload))
    },
