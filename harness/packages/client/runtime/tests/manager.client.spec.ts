...
describe('outline', () => {
  it('returns the whole-log turn rows and forwards the session id and signal', async () => {
    const api = new FakeApiClient()
    api.onOutline = () => Promise.resolve(ok({
      turns: [{ turn: 1, anchorSeq: 1, time: 0, userText: 'hi', replyText: 'hello', status: 'closed', hasToolCalls: false }],
    }))
    const manager = new SessionManager(api, fakeRemote())
    const signal = new AbortController().signal

    await expect(manager.outline(S1, signal)).resolves.toEqual({
      ok: true,
      value: {
        turns: [{ turn: 1, anchorSeq: 1, time: 0, userText: 'hi', replyText: 'hello', status: 'closed', hasToolCalls: false }],
      },
    })
    expect(api.callsOf('session.outline')).toEqual([{ sessionId: S1 }])
    expect(api.lastSearchSignal).toBe(signal)
  })
  ...
  it('preserves business errors and folds transport failures', async () => {
    ...
    const manager = new SessionManager(api, fakeRemote())
    const signal = new AbortController().signal
    api.onOutline = () => Promise.resolve(err({
      code: 'session-not-found',
      message: 'nope',
      details: { sessionId: S1 },
    }))
    await expect(manager.outline(S1, signal)).resolves.toMatchObject({
      ok: false,
      error: { code: 'session-not-found', message: 'nope' },
    })

    api.onOutline = () => Promise.reject(new Error('wire down'))
    await expect(manager.outline(S1, signal)).resolves.toMatchObject({
      ok: false,
      error: { code: 'internal', message: 'wire down' },
    })
