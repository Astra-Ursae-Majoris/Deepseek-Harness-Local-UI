...
function makeHarness(init?: Partial<ConversationSnapshot>) {
  ...
  const props: ChatViewSlotProps = {
    ...
    forkAt,
    forkAndSend: () => {},
    // Whole-log outline: absent turns fall back to the loaded-window summaries.
    outline: () => Promise.resolve({ ok: true as const, value: { turns: [] } }),
...
describe('ChatView', () => {
  ...
  it('renders the fixture main line as independently keyed business nodes', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    // Scope to the flow column: the resident outline sidebar mirrors turn text.
    const flow = view.container.querySelector('[data-chat-flow]') as HTMLElement
    expect(within(flow).getByText('do the thing')).toBeTruthy()
    expect(within(flow).getByText('running tools')).toBeTruthy()
  ...
  it('withholds assistant IconActions while the turn is still running', () => {
    ...
    // Scope to the flow column: the resident outline sidebar mirrors turn text.
    const flow = view.container.querySelector('[data-chat-flow]') as HTMLElement
    expect(within(flow).getByText('mid-turn text')).toBeTruthy()
  ...
  it('one ResizeObserver owns pinned dynamic-height follow and ignores growth while away', () => {
    // Two observers exist now: the dynamic-height follow (constructed first)
    // and the narrow-window auto-collapse (constructed second). The follow
    // assertions drive the FIRST callback; the second is left untouched.
    ...
    act(() => { callbacks[0]?.([], null as unknown as ResizeObserver) })
    expect(scroller.scrollTop).toBe(1_200)
    readerScroll(scroller, 200)
    Object.defineProperty(scroller, 'scrollHeight', { value: 1_400, writable: true })
    act(() => { callbacks[0]?.([], null as unknown as ResizeObserver) })
    expect(scroller.scrollTop).toBe(200)
    // One follow observe (column) plus one auto-collapse observe (root).
  ...
  it('keeps /compact no-history and error settlements on the generic command row', () => {
    ...
    // The row carries no controls; the view-level outline toggle is expected.
    const noHistoryRow = noHistoryView.getByText('No compactable history yet.').closest('[data-chat-flow-key]')
    expect(noHistoryRow?.querySelectorAll('button').length ?? -1).toBe(0)
...
describe('ChatView outline integration', () => {
  it('jumps to an older turn from the resident outline, saving the reader position', () => {
    const h = makeHarness({
      nodes: [
        user(1, '第一问'), assistant(2, '第一答', 0),
        user(3, '第二问'), assistant(4, '第二答', 1),
      ],
      turnEnds: new Map([[0, 2], [1, 4]]),
      turnTimings: new Map([[0, { startTime: 1000 }], [1, { startTime: 3000 }]]),
    })
    const save = vi.spyOn(h.chatScroll, 'save')
    const view = render(<h.ChatView {...h.props} />)
    fireEvent.click(view.getByText('第 1 条'))
    expect(save).toHaveBeenCalled()
  })

  it('collapses the resident outline to the rail and expands it again', () => {
    const h = makeHarness({
      nodes: [user(1, '第一问'), assistant(2, '第一答', 0)],
      turnEnds: new Map([[0, 2]]),
    })
    const view = render(<h.ChatView {...h.props} />)
    fireEvent.click(view.getByRole('button', { name: '收起导航' }))
    expect(view.queryByText('第 1 条')).toBeNull()
    fireEvent.click(view.getByRole('button', { name: '对话导航' }))
    expect(view.getByText('第 1 条')).toBeTruthy()
  })


  it('jumps directly to a server-outline turn already inside the window', async () => {
    const h = makeHarness({
      nodes: [user(1, '第一问'), assistant(2, '第一答', 0)],
      turnEnds: new Map([[0, 2]]),
    })
    h.props.outline = () => Promise.resolve({
      ok: true as const,
      value: { turns: [{ turn: 0, anchorSeq: 1, time: 0, userText: '第一问', replyText: '第一答', status: 'closed', hasToolCalls: false }] },
    })
    const save = vi.spyOn(h.chatScroll, 'save')
    const view = render(<h.ChatView {...h.props} />)
    // The server-outline row is the click target; the window fallback row
    // would carry the same 第 1 条 label, so click its distinctive text.
    fireEvent.click(await view.findByText('第一问'))
    expect(save).toHaveBeenCalled()
  })

  it('pages in older history when the target turn is outside the window', async () => {
    const h = makeHarness({
      nodes: [user(100, '窗口内的对话'), assistant(101, '回答', 5)],
      hasMore: true,
    })
    h.props.outline = () => Promise.resolve({
      ok: true as const,
      value: { turns: [{ turn: 0, anchorSeq: 1, time: 0, userText: '很早的对话', replyText: '', status: 'closed', hasToolCalls: false }] },
    })
    const loadOlder = vi.fn()
    h.props.loadOlder = loadOlder
    const view = render(<h.ChatView {...h.props} />)
    // Wait for the server-outline row (its anchor seq sits far below the
    // window, so the jump must page history in).
    fireEvent.click(await view.findByText('很早的对话'))
    expect(loadOlder).toHaveBeenCalled()
  })

  it('jumps back to latest from the outline', () => {
    const h = makeHarness({
      nodes: [user(1, '第一问'), assistant(2, '第一答', 0)],
      turnEnds: new Map([[0, 2]]),
    })
