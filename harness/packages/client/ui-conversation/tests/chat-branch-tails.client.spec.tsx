...
describe('MessageIconActions rewind/regenerate', () => {
  it('renders the two actions between copy and branch and fires their callbacks', () => {
    ...
    expect(screen.getByRole('button', { name: '回退到这一步' })).toBeTruthy()
    expect(screen.getByRole('button', { name: '重新生成' })).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '回退到这一步' }))
    expect(onRewind).toHaveBeenCalledTimes(1)
    fireEvent.click(screen.getByRole('button', { name: '重新生成' }))
    expect(onRegenerate).toHaveBeenCalledTimes(1)
  })

  it('hides the actions when the callbacks are omitted', () => {
    const writeText = vi.fn().mockResolvedValue(undefined)
    Object.defineProperty(navigator, 'clipboard', { configurable: true, value: { writeText } })
    render(<MessageIconActions text="原文" clock="start" t={t} />)
    expect(screen.queryByRole('button', { name: '回退到这一步' })).toBeNull()
    expect(screen.queryByRole('button', { name: '重新生成' })).toBeNull()
  })
})

describe('UserMessageNodeView rewind/regenerate wiring', () => {
  const EMPTY_TURNS = { chat: { legacy: { turnEnds: new Map<number, number>() } } }
  const TURNS = { chat: { legacy: { turnEnds: new Map([[0, 2]]) } } }
  const useSessionWith = (snapshot: unknown) => (selector: (s: unknown) => unknown) => selector(snapshot)

  function userNodeView(overrides: Partial<ChatConversationViewNode> = {}): ChatConversationViewNode {
    return {
      key: 'fixture:user:3',
      id: '3',
      target: 'chat',
      ...
      visibility: 'visible',
      data: { kind: 'user', seq: 3, time: 3000, content: [{ type: 'text', text: '重新生成我' }], source: null },
      ...overrides,
    }
  }

  it('shows both actions when a previous completed turn exists', () => {
    const forkAt = vi.fn()
    const forkAndSend = vi.fn()
    render(
      <UserMessageNodeView
        {...({
          node: userNodeView(),
          ...
          forkAt,
          forkAndSend,
          useSession: useSessionWith(TURNS),
          loadImage: async () => '',
        } as unknown as ChatNodeViewProps<'user'>)}
      />,
    )
    fireEvent.click(screen.getByRole('button', { name: '回退到这一步' }))
    expect(forkAt).toHaveBeenCalledWith(2)
    fireEvent.click(screen.getByRole('button', { name: '重新生成' }))
    expect(forkAndSend).toHaveBeenCalledWith(2, '重新生成我')
  })

  it('hides both actions on the first turn or without a completed predecessor', () => {
    render(
      <UserMessageNodeView
        {...({
          node: userNodeView(),
          ...
          forkAt: vi.fn(),
          forkAndSend: vi.fn(),
          useSession: useSessionWith(EMPTY_TURNS),
          loadImage: async () => '',
        } as unknown as ChatNodeViewProps<'user'>)}
      />,
    )
  ...
  it('hides regenerate for image-only messages; rewind stays', () => {
    const imageNode = userNodeView()
    ;(imageNode.data as { content: unknown[] }).content = [{ type: 'image' }]
    render(
      <UserMessageNodeView
        {...({
          node: imageNode,
...
function MessageItem({ node, t: translate }: MessageItemProps) {
  ...
  const props = {
    ...
    forkAt: () => {},
    forkAndSend: () => {},
    useSession: (selector: (s: unknown) => unknown) => selector({ chat: { legacy: { turnEnds: new Map<number, number>() } } }),
