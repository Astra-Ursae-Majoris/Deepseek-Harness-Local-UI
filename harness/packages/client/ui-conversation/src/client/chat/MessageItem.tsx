...
export const UserMessageNodeView = memo(function UserMessageNodeView({
  node, loadImage, forkAt, forkAndSend, useSession, t,
}: ChatNodeViewProps<'user' | 'steering'>) {
  const data = node.data
  // Rewind target: the end seq of the completed turn BEFORE this message's
  // turn (the fork API cuts at the first turn/end at or after the anchor).
  // User nodes carry no turn field; the view location supplies it.
  const rewindTarget = useSession(snapshot => {
    const location = node.location
    const turn = location.kind === 'turn' || location.kind === 'step' ? location.turn.turn : undefined
    return turn === undefined ? undefined : snapshot.chat.legacy.turnEnds.get(turn - 1)
  })
    ...
    <UserStyleBubble
      ...
      actions={bubbleText => (
        <MessageIconActions
          ...
          className={css.actions}
          t={t}
          onRewind={rewindTarget === undefined ? undefined : () => { forkAt(rewindTarget) }}
          onRegenerate={rewindTarget === undefined || text === '' || images.length > 0
            ? undefined
            : () => { forkAndSend(rewindTarget, text) }}
        />
      )}
    />
  )
