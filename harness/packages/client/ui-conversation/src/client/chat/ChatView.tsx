...
import { ConversationNav } from './ConversationNav.tsx'
import {
  buildTurnSummaries, outlineRowsFromTurns, outlineRowsFromWindow, type OutlineRow,
...
export function ChatView({
  useSession, useSessions, useStore, renderSlot, sessionId, openFile, loadOlder, loadImage, inspectCall, chatScroll, forkAt, forkAndSend,
  outline, fileMentions, t,
}: ChatViewSlotProps) {
  ...
  // Whole-log outline rows (null = window-summary fallback in use).
  const [outlineRows, setOutlineRows] = useState<readonly OutlineRow[] | null>(null)
  const [outlineLoading, setOutlineLoading] = useState(true)
  /** A jump target whose turn is outside the loaded window; paged in. */
  const [pendingJump, setPendingJump] = useState<{ seq: number } | null>(null)
  ...
  const scrollToKey = (key: string): void => {
    ...
    if (row === null) return
    const centered = el.scrollTop + flowTop(row, el) - el.clientHeight / 2
    el.scrollTop = Math.max(0, centered)
    observedTopRef.current = el.scrollTop
    atBottomRef.current = false
    setAtBottom(false)
    const flash = css.navFlash
    if (flash !== undefined) {
  ...
  // Narrow windows auto-collapse the outline rail so the transcript keeps
  // a usable width (mirrors the left sidebar's collapse affordance).
  const [autoCollapsed, setAutoCollapsed] = useState(false)
  useEffect(() => {
    const root = rootRef.current
    if (root === null || typeof ResizeObserver === 'undefined') return
    const observer = new ResizeObserver(() => {
      setAutoCollapsed(root.getBoundingClientRect().width < 760)
    })
    observer.observe(root)
    return () => { observer.disconnect() }
  }, [])

  // Whole-log outline: fetch on mount/session switch; a failed fetch falls
  // back to the loaded-window summaries below.
  useEffect(() => {
    let cancelled = false
    setOutlineLoading(true)
    void outline().then((result) => {
      if (cancelled) return
      setOutlineLoading(false)
      setOutlineRows(result.ok ? outlineRowsFromTurns(result.value.turns) : null)
    })
    return () => { cancelled = true }
  }, [sessionId, outline])

  // Refresh quietly when the live session crosses a turn boundary (a new
  // turn starts or a running one closes), so the tail stays current.
  const turnCount = useSession(snapshot => snapshot.chat.timeline.turnOrder.length)
  const liveSig = `${running ? 1 : 0}:${turnCount}`
  const lastLiveSig = useRef(liveSig)
  useEffect(() => {
    if (lastLiveSig.current === liveSig) return
    lastLiveSig.current = liveSig
    void outline().then((result) => {
      if (result.ok) setOutlineRows(outlineRowsFromTurns(result.value.turns))
    })
  }, [liveSig, outline])

  // Jump choreography: covered targets scroll directly; uncovered ones page
  // older history in until the window covers the anchor seq.
  const firstNodeKeyAtOrAfter = (seq: number): string | null => {
    for (const key of order) {
      const node = nodeStore.get(key)
      if (node !== undefined && node.anchorSeq >= seq) return key
    }
    return null
  }
  useEffect(() => {
    if (pendingJump === null) return
    const { seq } = pendingJump
    const first = order[0]
    const firstSeq = first === undefined ? null : nodeStore.get(first)?.anchorSeq ?? null
    if (firstSeq !== null && firstSeq <= seq) {
      const key = firstNodeKeyAtOrAfter(seq)
      if (key !== null) {
        setPendingJump(null)
        scrollToKey(key)
        return
    ...
    else if (!hasMore) setPendingJump(null)
  })
  ...
  return (
    <div ref={rootRef} className={css.root}>
      ...
      <ConversationNav
        rows={outlineRows ?? outlineRowsFromWindow(buildTurnSummaries(chat))}
        loading={outlineLoading && outlineRows === null}
        forceCollapsed={autoCollapsed}
        onJump={(seq) => { jumpToTurn(seq) }}
        toLatest={toLatest}
        t={t}
      />
    </div>
