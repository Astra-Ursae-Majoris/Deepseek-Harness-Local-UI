...
import type {
  ...
  ObservableSnapshot, OutlineTurn, PendingInteraction, PendingWait, SessionId, ToolCallBlock,
  TurnLocation, WorkspaceId,
} from '@deepseek-ai/dsh-client-runtime/client'
import type { MarkdownFileMentions } from '@deepseek-ai/dsh-client-ui-primitives'
import type { MessageId, RpcResult } from '@deepseek-ai/dsh-client-connection/client'
...
export interface ChatNodeOwnerProps {
  ...
  /** Fork before the completed turn ending at `seq`, open the child, and resend `text` into it. */
...
export interface ChatViewInjected {
  ...
  forkAndSend: (seq: number, text: string) => void
  /**
   * Fetch the full conversation outline (one row per turn), independent of
   * the loaded window, so the sidebar can list and jump to any turn.
   */
  outline: () => Promise<RpcResult<{ turns: OutlineTurn[] }>>
  /**
