...
import {
  resolveWorkspacePath, type ISessions, type SessionFace, type SessionId,
  ...
  slots.register({
    ...
    inject: (sessionId: SessionId, actions: BoundActions<typeof chatStore>): ChatViewInjected => {
      ...
      return {
        ...
        outline: () => sessions.outline(sessionId, new AbortController().signal),
        forkAndSend: (seq, text) => {
          void (async () => {
            let childId: SessionId
            try {
              childId = await sessions.fork({ sessionId, atSeq: seq, increaseTitle: true })
            } catch {
              return // Fork failure keeps the source view untouched (same as forkAt).
            }
            ...
            const conversation = ctx.get('conversation') as
              ...
              | undefined
            /* v8 ignore next -- the conversation service is registered by this plugin's own apply. */
            if (binding === undefined || conversation === undefined) return
            try {
              await conversation.sendSession(binding.session, text, [], 'queue')
            } catch {
              // Resend failure leaves the child open; the composer surfaces it.
            }
          })()
        },
      }
