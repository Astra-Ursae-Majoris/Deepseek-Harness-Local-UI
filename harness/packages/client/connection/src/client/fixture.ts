...
import type { OutlineTurn, RequestPayload, ResponseValue, RpcMethodMap } from '@deepseek-ai/dsh-host-apiproxy/api'
...
/** Joined text blocks of one message payload (outline rows reuse the same vocabulary). */
function fixtureMessageText(message: { readonly content?: readonly unknown[] }): string {
  let out = ''
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    sessions: {
      ...
      outline: (request, signal) => {
        if (signal.aborted) {
          return err(request, {
            code: 'cancelled',
            message: 'fixture session outline was aborted',
            details: {},
          })
        }
        ...
        for (const event of log) {
          switch (event.type) {
            ...
            case 'user/message': {
              ...
              if (current.userText === '') current.userText = fixtureMessageText(event.data)
              break
            }
            case 'assistant/message': {
              if (current === undefined) break
              if (current.replyText === '') current.replyText = fixtureMessageText(event.data.message)
              break
            }
            case 'tool/call': {
              if (current === undefined) break
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'session.outline': return this.api.sessions.outline(request, signal)
